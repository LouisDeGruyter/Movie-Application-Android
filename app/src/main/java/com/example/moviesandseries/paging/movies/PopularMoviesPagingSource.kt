...
/**
 * [PagingSource] implementation for loading paginated popular movies data.
 *
 * @param movieRepository The repository for fetching popular movies data.
 */
class PopularMoviesPagingSource(private val movieRepository: MovieRepository) :
    PagingSource<Int, MediaIndex>() {

    /**
     * Load function that is called to load a chunk of popular movies data based on the provided [params].
     *
     * @param params Parameters for loading data, including the requested page key.
     * @return [LoadResult<Int, MediaIndex>] containing the loaded popular movies data, with optional adjacent keys.
     */
    ...
    /**
     * Returns the key for refreshing the popular movies data based on the current [state].
     *
     * @param state The current [PagingState].
     * @return The key used to refresh the popular movies data.
     */
