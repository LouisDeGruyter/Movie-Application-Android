...
class SimilarMoviesPagingSource (private val movieRepository: MovieRepository, private val movieId: Int): PagingSource<Int, MovieIndex>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MovieIndex> {
        return try {
            val page = params.key ?: 1
            val response = movieRepository.getSimilarMovies(movieId= movieId,page=page)
            LoadResult.Page(
