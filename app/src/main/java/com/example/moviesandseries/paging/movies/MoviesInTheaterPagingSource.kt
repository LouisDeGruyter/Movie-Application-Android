...
class MoviesInTheaterPagingSource (private val movieRepository: MovieRepository): PagingSource<Int, MovieIndex>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MovieIndex> {
        return try {
            val page = params.key ?: 1
            val response = movieRepository.getMoviesInTheaters(page=page)
            LoadResult.Page(
